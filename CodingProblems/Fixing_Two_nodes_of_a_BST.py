...
class Solution:
    ...
    def correctBSTUtil(self, root):
        if root is None:
            return

        self.correctBSTUtil(root.left)

        if self.previousNode is not None and root.data < self.previousNode.data:
        ...
        self.correctBSTUtil(root.right)

    def correctBST(self, root):
        # code here
        self.correctBSTUtil(root)

        if self.firstNode is not None:
            temp = self.firstNode.data
            self.firstNode.data = self.secondNode.data
            self.secondNode.data = temp
